...
export const PROJECTS: Project[] = [
  {
    ...
    bullets: [
      ...
      'RAG pipeline: sanitize input → JWT authCheck + 30 req/min rate limit → parallel(TF-IDF score against projectKnowledge.json → top 8 chunks, regex intent classifier → live MongoDB queries) → buildSystemPrompt() assembles behaviour rules + live data block + RAG chunks + conversation history → Azure call with tool_choice:"auto" → optional tool call for filtered DB queries → SSE stream to browser',
